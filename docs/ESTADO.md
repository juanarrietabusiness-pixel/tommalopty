...
> **Última actualización:** 6 de septiembre de 2026 (cinco issues cerrados ese
> día —#9, #10, #11, #46 y #54—; staging publicado al día; queda la clave
> maestra).
...
> número 4). Sobre los tipos: **no hay deriva de esquema** —la sección `public`
> del diff de CI está vacía—; lo que sobraba eran los esquemas `graphql_public` y
> `storage`. Se alineó la generación a `--schema public` en los dos lados; **una
> ronda de CI confirma si el diff queda ya vacío**
...
| #   | Qué                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 9   | ~~**El buzón abierto de `leads`** y **el checkout de invitado con correo ajeno**~~ **Resuelto (6 sep, PR [#65](https://github.com/juanarrietabusiness-pixel/tommalopty/pull/65)).** `anon` ya no escribe en `leads` desde PostgREST —ese era el agujero, no la falta de límite—, y el límite vive ahora en Postgres, no en la ruta: diez altas por IP y hora, guardando el hash de la IP y nunca la IP. En el checkout, un invitado con el correo de una cuenta registrada ya no se cuelga de su ficha ([#9](https://github.com/juanarrietabusiness-pixel/tommalopty/issues/9), [#10](https://github.com/juanarrietabusiness-pixel/tommalopty/issues/10)). **Cambia un comportamiento visible:** quien esté registrado y compre sin iniciar sesión no verá ese pedido en «Mis pedidos» — le llega por correo con su enlace de seguimiento, y el equipo lo ve entero en el panel |
...
| 13  | ~~**`/cuenta/direcciones` es solo lectura**~~ **Resuelto.** La pantalla ya guarda, edita y borra direcciones con su punto (`LibretaDeDirecciones`, con su Server Action). Decía «la que uses en tu próximo pedido aparecerá aquí» y era mentira: el checkout guarda la dirección como copia dentro del pedido, así que comprar no llenaba nunca esta lista                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 15  | ~~**La pantalla de integraciones lee `NEXT_PUBLIC_META_PIXEL_ID` de forma dinámica**~~ **Resuelto.** `entornoDeCredenciales()` (`apps/admin/src/lib/credenciales.ts`) escribe cada nombre literal, que es lo único que Next sustituye — la lección de § 4, aplicada                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
### La garantía de que una tabla nace con RLS vive fuera del repositorio

Lo que cerró el [#11](https://github.com/juanarrietabusiness-pixel/tommalopty/issues/11) es un disparador de eventos DDL,
`ensure_rls` (`ddl_command_end` → `public.rls_auto_enable`): activa RLS sobre
cualquier tabla nueva de `public` en el momento de crearla. Según la nota con
la que se cerró el issue, se comprobó en vivo contra staging el 6 de septiembre
dentro de un `begin … rollback`: una tabla creada sin pedirlo salió con
`relrowsecurity = true`, y ese día no había ni una tabla de `public` sin RLS.

**Pero ese disparador no lo crea ninguna migración.** Se le menciona en los
comentarios de `20260830040000_endurecer_funciones.sql` —donde se explica por qué
la revocación masiva de `EXECUTE` tiene que dejarlo fuera— y no aparece en ningún
...
(nadie ve nada), no el estado útil. La política sigue siendo trabajo de la
migración; lo que ya no puede pasar es que una tabla nazca abierta de par en par.
