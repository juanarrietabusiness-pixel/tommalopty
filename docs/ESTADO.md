...
| 9b  | **Las cinco vistas conservan privilegios para `anon`** —`INSERT`, `TRIGGER`, `REFERENCES` y `TRUNCATE`—: la revocación de la migración 0033 fue tabla por tabla y no alcanzó a las vistas. **No es explotable hoy** (las cinco tienen `security_invoker=on`, y se comprobó que `anon` sobre `report_sales_daily` da `permission denied for table orders`), pero rompe el invariante escrito. El `SELECT` de `product_catalog` **hay que conservarlo** ([#38](https://github.com/juanarrietabusiness-pixel/tommalopty/issues/38)) |
| 9c  | **Huecos de cobertura**: `integration_credentials` no tiene **ni un test** pese a guardar todas las credenciales futuras; el arreglo de #8 entró sin red; y «una tabla nueva nace sin nada para anon» solo valida el ACL de `postgres`, no el de `supabase_admin`, que sigue concediéndolo todo ([#39](https://github.com/juanarrietabusiness-pixel/tommalopty/issues/39))                                                                                                                                                       |
